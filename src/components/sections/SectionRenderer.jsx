...
export default function SectionRenderer({ section, site }) {
  ...
  return <section className={`${pad} px-4`} style={{ background: section.background || 'transparent' }}><div className="mx-auto max-w-5xl">
    ...
    {section.type === 'cards' && <div><h2 className="text-3xl font-bold">{section.title}</h2><div className="mt-6 grid gap-4 md:grid-cols-3">{(c.cards||[]).map(card=><div key={card.id} className="rounded-[var(--radius)] border bg-white p-5 shadow-sm"><h3 className="font-bold">{card.title}</h3><p className="mt-2 text-sm text-gray-600">{card.text}</p></div>)}</div></div>}
